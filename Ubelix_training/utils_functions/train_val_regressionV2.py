...
def train_regressionV2(model, train_dataloader, test_dataloader,
    ...
    for epoch in range(n_epochs):
        ...
        output_dir = 'models/regression/{}'.format(fileExtension)
        mkdir_p(output_dir)
        torch.save(model.state_dict(),'{}/{}_{}epoch_{}_Temp_train_{}_{}batchs_{}epochs_Regression.pth'.format(output_dir_model, fileExtension, epoch, date4File,
                                                                                                      cubeSetName,
                                                                                                      str(batch_size),
                                                                                                      str(n_epochs),
                                                                                                      ))
    ...
    for i in range(0, nim):
        ...
        plt.xticks([0, 512])
        plt.yticks([])
    plt.savefig('{}/image_regression_{}batch_{}epochs_{}.png'.format(output_dir_results, batch_size, n_epochs, fileExtension), bbox_inches = 'tight', pad_inches = 0.05)
    # -----------plot and save section ------------------------------------------------------------------------------------

    fig, (p1, p2, p4) = plt.subplots(3, figsize=(15, 10))  # largeur hauteur
    fig.suptitle("Regression,  training {} epochs with {} images, lr={} ".format(n_epochs, numbOfImageDataset, lr), fontsize=14)
    ...
    fig.savefig('{}/regression_{}batch_{}epochs_{}.png'.format(output_dir_results,batch_size, n_epochs, fileExtension), bbox_inches = 'tight', pad_inches = 0.05)
    import matplotlib2tikz

    matplotlib2tikz.save("{}/regression_{}batch_{}epochs_{}.tex".format(output_dir_results, batch_size, n_epochs, fileExtension),figureheight='5.5cm', figurewidth='15cm')

    torch.save(model.state_dict(),
               '{}/{}_{}_FinalModel_train_{}_{}batchs_{}epochs_Regression.pth'.format(output_dir_model, fileExtension, date4File,
                                                                                        cubeSetName,
                                                                                        str(batch_size),
                                                                                        str(n_epochs),
                                                                                        ))
